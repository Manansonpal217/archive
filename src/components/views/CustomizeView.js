...
export class CustomizeView extends LitElement {
    ...
    constructor() {
        ...
        this.selectedModel = 'llama3.2:3b';
    ...
    renderModelSection() {
        const models = [
            { value: 'llama3.2:3b', name: 'Llama 3.2 3B (Fast)' },
            { value: 'llama3.2', name: 'Llama 3.2' },
            { value: 'gpt-oss:120b-cloud', name: 'GPT-OSS 120B Cloud' },
            { value: 'gpt-oss:20b-cloud', name: 'GPT-OSS 20B Cloud' },
            { value: 'deepseek-v3.1:671b-cloud', name: 'DeepSeek V3.1 671B Cloud' },
            { value: 'qwen3-coder:480b-cloud', name: 'Qwen3 Coder 480B Cloud' },
            { value: 'qwen3-vl:235b-cloud', name: 'Qwen3 VL 235B Cloud' },
            { value: 'minimax-m2:cloud', name: 'Minimax M2 Cloud' },
            { value: 'glm-4.6:cloud', name: 'GLM 4.6 Cloud' },
        ];

        return html`
            <div class="content-header">Model Selection</div>
            <div class="settings-section">
                <div class="section-title">Ollama Model</div>
                <div class="form-grid">
                    <div class="form-group">
                        <label class="form-label">
                            Select Model
                            <span class="current-selection">${this.selectedModel}</span>
                        </label>
                        <select class="form-control" .value=${this.selectedModel} @change=${this.handleModelSelect}>
                            ${models.map(
                                model => html`
                                    <option value=${model.value} ?selected=${this.selectedModel === model.value}>
                                        ${model.name}
                                    </option>
                                `
                            )}
                        ...
                        <div class="form-description">
                            Choose which Ollama model to use for text responses. Cloud models require Ollama Cloud authentication.
                            <br><br>
                            <strong>Local models</strong> (like llama3.2:3b) run faster and work offline.
                            <br>
                            <strong>Cloud models</strong> are more capable but require internet and Ollama Cloud sign-in.
                        </div>
                    </div>
                </div>
            </div>
