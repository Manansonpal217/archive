...
function setupStorageIpcHandlers() {
    ...
    ipcMain.handle('storage:get-assembly-api-key', async () => {
        try {
            return { success: true, data: storage.getAssemblyApiKey() };
        } catch (error) {
            console.error('Error getting AssemblyAI API key:', error);
            return { success: false, error: error.message };
        }
    });

    ipcMain.handle('storage:set-assembly-api-key', async (event, apiKey) => {
        try {
            storage.setAssemblyApiKey(apiKey);
            return { success: true };
        } catch (error) {
